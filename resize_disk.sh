...
[ -z "$OUTPUT_FORMAT" ] && OUTPUT_FORMAT="raw" && echo "警告：未知格式，默认使用 raw"
...
get_partition_size_bytes() {
    echo "$FS_INFO" | awk -v name_col="$COL_NAME" -v size_col="$COL_SIZE" -v part="$1" '
...
calc_expand_bytes() {
    ...
    if [[ "$size_str" =~ ^([0-9]+)%$ ]]; then
        ...
        expand_bytes=$(( target_bytes > current_bytes ? target_bytes - current_bytes : 0 ))
    fi
...
# ---------- 自动选择分区函数 ----------
is_lv() {
    echo "$LV_LIST" | awk '{print $1}' | grep -Fxq "$1"
}

find_expand_partition() {
    local pv_list=$(virt-filesystems -a "$ORIGINAL_NAME" --pv 2>/dev/null)
    if [ -n "$pv_list" ]; then
        local pv_count=$(echo "$pv_list" | grep -c .)
        ...
        local pv=$(echo "$pv_list" | head -n1)
        local is_part=$(echo "$FS_INFO" | awk -v name_col="$COL_NAME" -v type_col="$COL_TYPE" -v p="$pv" '
        ...
        local lvm_devs=$(echo "$FS_INFO" | awk -v vfs_col="$COL_VFS" -v name_col="$COL_NAME" '
    ...
    local partitions=$(echo "$FS_INFO" | awk -v type_col="$COL_TYPE" -v name_col="$COL_NAME" '
        NR>1 && $type_col == "partition" {print $name_col}
    ')
    [ -z "$partitions" ] && return 1

    local first_part=$(echo "$partitions" | head -n1)

    # 按优先级尝试
    local result
    result=$(echo "$FS_INFO" | awk -v name_col="$COL_NAME" -v label_col="$COL_LABEL" -v wl="$label_whitelist" '
        BEGIN { split(wl, arr, " "); for (i in arr) whitelist[arr[i]] = 1 }
        NR>1 {
            label = tolower($label_col); gsub(/^[ \t]+|[ \t]+$/, "", label)
            if (whitelist[label]) { print $name_col; exit }
        }
    ')
    ...
    result=$(echo "$FS_INFO" | awk -v name_col="$COL_NAME" -v vfs_col="$COL_VFS" -v type_col="$COL_TYPE" -v label_col="$COL_LABEL" -v size_col="$COL_SIZE" \
        ...
        BEGIN { split(vfs_bl, vfs_arr, " "); for (i in vfs_arr) vfs_blacklist_map[vfs_arr[i]] = 1 }
        NR>1 && $type_col == "partition" {
            name = $name_col; vfs = tolower($vfs_col); size = $size_col + 0
            label = tolower($label_col); gsub(/^[ \t]+|[ \t]+$/, "", label)
            if (vfs_blacklist_map[vfs]) next
            if (label ~ /boot|efi|esp/) next
            if (size <= min_sz) next
    ...
    result=$(echo "$FS_INFO" | awk -v name_col="$COL_NAME" -v vfs_col="$COL_VFS" -v type_col="$COL_TYPE" -v size_col="$COL_SIZE" -v min_sz="$min_size" '
        NR>1 && $type_col == "partition" {
            vfs = tolower($vfs_col); size = $size_col + 0
...
# 辅助函数：处理一个分区的扩容规则（带有操作符和大小）
# 参数: partition operator size_str
# 输出: 返回扩容字节数，同时修改全局变量 RESIZE_OPTS / LV_EXPAND / EXPAND_PARTITION（若需要）
process_partition_rule() {
    local partition="$1"
    local operator="$2"
    local size_str="$3"
    local current_bytes size_bytes
    local action_text

    current_bytes=$(get_partition_size_bytes "$partition")
    size_bytes=$(calc_expand_bytes "$size_str" "$current_bytes")
    ...
    if is_lv "$partition"; then
        ...
        LV_EXPAND="$partition"
        echo "规则: LV $partition $action_text $size_str"
    else
        RESIZE_OPTS="$RESIZE_OPTS --resize ${partition}=${operator}${size_str}"
        echo "规则: 分区 $partition $action_text $size_str"
    fi
...
# 处理自动选择规则（无设备名，如 "2G", "+10%", "=10G"）
process_auto_rule() {
    local rule="$1"

    if [[ "$rule" =~ ^[0-9]+[GMK]?$ ]]; then
        local bytes=$(parse_size_to_bytes "$rule")
        if [ "$bytes" -gt 0 ]; then
        ...
        EXPAND_PARTITION=$(find_expand_partition)
        if [ -n "$EXPAND_PARTITION" ]; then
            local current_bytes=$(get_partition_size_bytes "$EXPAND_PARTITION")
            local size_bytes=$(calc_expand_bytes "$size" "$current_bytes")
            local action_text=$([ "$operator" == "+" ] && echo "增加" || echo "设为")
            RESIZE_OPTS="$RESIZE_OPTS --resize ${EXPAND_PARTITION}=${operator}${size}"
            echo "规则: 自动选择分区 $EXPAND_PARTITION，$action_text $size"
            echo "$size_bytes"
        else
            echo "0"
        fi
    else
        echo "错误：无法解析规则 '$rule'" >&2
        exit 1
    fi
}

for i in "${!RULES[@]}"; do
    rule=$(echo "${RULES[$i]}" | xargs)
    is_last=$(( i + 1 == RULE_COUNT ? 1 : 0 ))

    if [[ "$rule" =~ ^(/dev/[^+=%]+)([+=])(.+)$ ]]; then
        # 形如 /dev/xxx+... 或 /dev/xxx=...
        bytes=$(process_partition_rule "${BASH_REMATCH[1]}" "${BASH_REMATCH[2]}" "${BASH_REMATCH[3]}")
        EXPAND_SIZE_BYTES=$((EXPAND_SIZE_BYTES + bytes))
    elif [[ "$rule" =~ ^/dev/ ]]; then
        # 纯设备名
        process_naked_device_rule "$rule" "$is_last"
    else
        # 自动规则
        bytes=$(process_auto_rule "$rule")
        EXPAND_SIZE_BYTES=$((EXPAND_SIZE_BYTES + bytes))
    fi
done
